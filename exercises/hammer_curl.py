...
class HammerCurl:
    ...
    def track_hammer_curl(self, landmarks, frame):
        ...
        # Set color based on whether the angle is in the correct range
        color_left = (0, 255, 0) if self.angle_threshold_down <= angle_left_counter <= self.angle_threshold_up else (0, 0, 255)
        color_right = (0, 255, 0) if self.angle_threshold_down <= angle_right_counter <= self.angle_threshold_up else (0, 0, 255)

        # Convert the angles to integers and update the text positions
        angle_text_position_left = (elbow_left[0] + 10, elbow_left[1] - 10)
        cv2.putText(frame, f'Angle: {int(angle_left_counter)}', angle_text_position_left, 
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_left, 2)

        angle_text_position_right = (elbow_right[0] + 10, elbow_right[1] - 10)
        cv2.putText(frame, f'Angle: {int(angle_right_counter)}', angle_text_position_right, 
